...
namespace Files.ClassLibrary.DataModels
{
    public class SidebarPinnedModel
    {
        ...
        public void AddItemToSidebar(string path)
        {
            try
            {
                var folder = Path.GetFileName(path);
                int insertIndex;
                if (MainPage.sideBarItems.Count <= 0)
                {
                    insertIndex = 0;
                }
                else
                {
                    insertIndex = MainPage.sideBarItems.IndexOf(MainPage.sideBarItems.Last(x => x.ItemType == NavigationControlItemType.Location
